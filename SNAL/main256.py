...
from models import *
from models.resnet import ResNet18_caltech256
...
parser.add_argument('--lr', default=0.05, type=float, help='learning rate')  #cifar100 0.05 cifar10 0.1 101 0.1 256 0.1
...
def get_sampler(strategyname, setname):
    ...
    if strategyname == 'random':
        ...
        return max_entropy_sample(set, net, cfg, dataset = setname, transforms = None)
    elif strategyname == 'bayesian':
    
        return bayesian_sample(set, net, cfg, dataset = setname, transforms = None)
    elif strategyname == 'siamese':
    
        return siamese_sample(set, net, cfg, dataset = setname, transforms = None)
...
def test(epoch):
    ...
    f0 = open('/home/lijianing/AL/pytorch-cifar/logs/logs256.txt', 'a')
    f0.write(str(acc))
    f0.write('  ')
    if epoch // 15 == 0:
        f0.write('\n')
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, './checkpoint/res256ckpt.pth') ####################33
        best_acc = acc
...
if __name__ == "__main__":    
    ...
    parser.add_argument('--dataset', default = 'caltech256', help = 'dataset_path_name')
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=200)
    '''
    sampler = get_sampler("random", "caltech256")
    sampler.query(2500)
    sampler.update()
    update(g_args)
    '''
    ...
    set, trainset, testset = get_set("caltech256")
