...
def test(epoch):
    ...
    f0 = open('/home/lijianing/AL/pytorch-cifar/logs/logs100.txt', 'a')
    f0.write(str(acc))
    f0.write('  ')
    if epoch // 15 == 0:
        f0.write('\n')
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, './checkpoint/res100ckpt.pth') ####################33
        best_acc = acc
...
if __name__ == "__main__":    
    ...
    parser.add_argument('--dataset', default = 'cifar100', help = 'dataset_path_name')
    ...
    parser.add_argument('--weight_dir', default = '/home/lijianing/AL/pytorch-cifar/*/100ckpt.pth', help = 'pretrained network parameters')
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=200)
    '''
    sampler = get_sampler("random", "cifar100")
    sampler.query(5000)
    sampler.update()
    update(g_args)    
    '''

    set, trainset, testset = get_set("cifar100")
    trainloader = torch.utils.data.DataLoader(
    trainset, batch_size=128, shuffle=False, num_workers=2)#36 caltech  #128 cifar 256 256
    ...
    for itr in range(1,7):    
        ...
        sampler = get_sampler("siamese", "cifar100")
        sampler.query(2500)
        sampler.update()
        update(g_args)
