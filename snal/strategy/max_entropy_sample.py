...
class max_entropy_sample(Strategy):
    def __init__(self, set, net, cfg, dataset, transform):
        super(max_entropy_sample, self).__init__(set, net, cfg, dataset, transform)
    ...
    def query(self, amount):
        ...
        for unlabel in tqdm(unlabel_list):
            ...
            ents[unlabel] = ent_sum
        
        ents_list = sorted(ents.items(), key = lambda x:x[1], reverse = False)   
