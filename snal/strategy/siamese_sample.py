...
class siamese_sample(Strategy):        # whether make thransforms to the compared data???
    def __init__(self, set, net, cfg, dataset, transform):
        super(siamese_sample,self).__init__(set, net, cfg, dataset, transform) #?
    ...
    def query(self, amount):
        ...
        for unlabel_tsr in tqdm(unlabel_tensor_list):
            ...
            for label_tsr in label_tensor_list[start:]:
                label_tensor = torch.load(label_tsr).to(device)
